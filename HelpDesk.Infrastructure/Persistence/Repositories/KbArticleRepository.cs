...
public class KbArticleRepository : GenericRepository<KbArticle>, IKbArticleRepository
{
    ...
    public async Task<IEnumerable<KbArticle>> GetByStatusAsync(KbArticleStatus status) =>
        await _context.KbArticles.Include(k => k.Author).Include(k => k.Category)
        .Where(k => k.Status == status).OrderByDescending(k => k.LastModifiedAt).ToListAsync();

    public async Task<IEnumerable<KbArticle>> SearchAsync(string keyword)
    {
        var term = keyword.ToLower();
        var title = await _context.KbArticles.Where(k => k.Status == KbArticleStatus.Published && k.Title.ToLower().Contains(term)).Include(k => k.Category).ToListAsync();
        var body = await _context.KbArticles.Where(k => k.Status == KbArticleStatus.Published && !k.Title.ToLower().Contains(term) && (k.Content.ToLower().Contains(term) || (k.Tags != null && k.Tags.ToLower().Contains(term)))).Include(k => k.Category).ToListAsync();
        return title.Concat(body);
    }

    public async Task<IEnumerable<KbArticle>> SuggestForTitleAsync(string title, int take = 3)
    {
        var words = title.Split(' ', StringSplitOptions.RemoveEmptyEntries).Where(w => w.Length > 3).Select(w => w.ToLower()).ToList();
        if (!words.Any()) return new List<KbArticle>();
        var articles = await _context.KbArticles.Where(k => k.Status == KbArticleStatus.Published).Include(k => k.Category).ToListAsync();
        return articles
