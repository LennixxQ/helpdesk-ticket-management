...
namespace HelpDesk.Infrastructure.Persistence.Repositories
{
    public class TicketRepository : GenericRepository<Ticket>, ITicketRepository
    {
        ...
        public async Task<IEnumerable<Ticket>> GetByAgentIdAsync(Guid agentId) =>
            await _context.Tickets.Include(t => t.Category).Include(t => t.RaisedByUser).Where(t => t.AssignedAgentId == agentId).OrderByDescending(t => t.CreatedAt).ToListAsync();
        ...
        public async Task<int> CountThisMonthAsync()
        {
            ...
            return await _context.Tickets.CountAsync(t => t.CreatedAt.Year == now.Year && t.CreatedAt.Month == now.Month);
        }

        public async Task<int> CountLastMonthAsync()
        {
            var last = DateTime.UtcNow.AddMonths(-1);
            return await _context.Tickets.CountAsync(t => t.CreatedAt.Year == last.Year && t.CreatedAt.Month == last.Month);
        }

        public async Task<List<(Guid AgentId, string AgentName, int Count)>> GetTopAgentsThisMonthAsync(int top = 5)
        {
            var now = DateTime.UtcNow;
            var result = await _context.Tickets.Where(t => t.Status == TicketStatus.Resolved &&t.AssignedAgentId != null && t.LastModifiedAt.HasValue && t.LastModifiedAt.Value.Year == now.Year && t.LastModifiedAt.Value.Month == now.Month)
                .GroupBy(t => t.AssignedAgentId).Select(g => new { AgentId = g.Key!.Value, Count = g.Count() }).OrderByDescending(x => x.Count).Take(top).ToListAsync();
